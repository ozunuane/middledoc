...
export function EditClientModal({ isOpen, onClose, onClientUpdated, client }: EditClientModalProps) {
  ...
  // Pre-fill form when modal opens or client changes
  useEffect(() => {
    if (isOpen && client) {
      setName(client.name)
    ...
    >
      <div className="bg-neutral-50 rounded-[16px] shadow-hero max-w-[560px] w-full mx-4">
        ...
        <div className="flex items-center justify-between px-7 pt-6 pb-4">
          <h2 className="font-serif text-[26px] text-neutral-900">Edit client</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit}>
          <div className="px-7 pb-6 space-y-4">
            ...
            <div>
              <label htmlFor="edit-client-name" className="block text-[13px] font-semibold text-neutral-700 mb-1.5">
                Name
              </label>
              <input
                ref={nameInputRef}
            ...
            <div>
              <label htmlFor="edit-client-email" className="block text-[13px] font-semibold text-neutral-700 mb-1.5">
                Email
              </label>
              <input
